...
def detect_anomalies(directory,anomaly_percentage):
    print('anomalies are detecting...')

    data = pd.read_csv(directory + "/merged_local_outlier_factor_file.csv")
    ...
    # count['Ranking_Factor'] = count['Count']*count['Count']*100*0.8 + \
    #                          count['Average_lof']*count['Average_lof']*0.2
    # count = count.sort_values(by=['Ranking_Factor'],ascending=False)
    count = count.sort_values(by=['Ranking_Factor'])

    count = count.head(6)

    count.to_csv(directory + "/anomalies.csv")
