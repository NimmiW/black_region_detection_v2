...
def calculate_features(time_period,root):
    print('features are calculating...')
    # price
    data = pd.read_csv(root + 'data/' + time_period.data_file)
    ...
    mask = (data.index > time_period.start_date) & (data.index <= time_period.end_date)
    data = data.loc[mask]
    series = data["Close"]
    series = series.rolling(window=60).mean()
    ...
    ax3.plot(series)
    ax3.set_title(time_period.currency + ' prices during ' + time_period.news + ' time period')
    ax3.set_xlabel('Time')
    ...
    print('p-value: %f' % ADF_result_log_return[
        1])  # p-value: 0.000000 therefore, null hypothesis is rejected. the system is stationary
    print('Critical Values:')
    ...
    print('p-value: %f' % ADF_result_residual_squared[
        1])  # p-value: 0.000000 therefore, null hypothesis is rejected. the system is stationary
    print('Critical Values:')
    ...
    ax1.plot(np_array_dates, gradients)
    ax1.set_title('Gradients: ' + time_period.currency + ' prices during ' + time_period.news)
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Gradient')"""
    ...
    directory = root + 'output/' + time_period.currency + str(time_now).replace(':','-')
